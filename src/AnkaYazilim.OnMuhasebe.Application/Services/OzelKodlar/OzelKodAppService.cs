namespace AnkaYazilim.OnMuhasebe.Services.OzelKodlar;
...
public class OzelKodAppService : OnMuhasebeAppService, IOzelKodAppService
{
    ...
    {
        var entities = await _ozelKodRepository.GetPagedListAsync(input.SkipCount, input.MaxResultCount, x => x.KodTuru == input.OzelKodTuru &&
         x.KartTuru == input.KartTuru && x.Durum == input.Durum, x => x.Kod);

        var totalCount = await _ozelKodRepository.CountAsync(x => x.KodTuru == input.OzelKodTuru &&
    ...
    {
        return await _ozelKodRepository.GetCodeAsync(x => x.Kod, x => x.KodTuru == input.OzelKodTuru && x.KartTuru == input.KartTuru && x.Durum == input.Durum);
    }
