...
public class FaturaAppService : OnMuhasebeAppService, IFaturaAppService
{
    ...
    public virtual async Task<PagedResultDto<ListFaturaDto>> GetListAsync(FaturaListParameterDto input)
    {
        ...
        x.SubeId == input.SubeId && x.DonemId == input.DonemId && x.Durum == input.Durum, x => x.FaturaNo, x => x.Cari.Unvan, x => x.OzelKod1, x => x.OzelKod2);
    ...
    {
        return await _repository.GetCodeAsync(x => x.FaturaNo, x => x.FaturaTuru == input.FaturaTuru && x.SubeId == input.SubeId && x.DonemId == input.DonemId && x.Durum == input.Durum);
    }
