namespace AnkaYazilim.OnMuhasebe.Services.BankaHesaplar;
...
public class BankaHesapAppService : OnMuhasebeAppService, IBankaHesapAppService
{
    ...
    public virtual async Task<PagedResultDto<ListBankaHesapDto>> GetListAsync(BankaHesapListParameterDto input)
    {
        var entities = await _bankaHesapRepository.GetPagedListAsync(input.SkipCount, input.MaxResultCount,
            ...
            x => x.Kod, x => x.BankaSube, x => x.BankaSube.Banka, x => x.OzelKod1, x => x.OzelKod2);

        var totalCount = await _bankaHesapRepository.CountAsync(x => input.HesapTuru == null ? x.SubeId == input.SubeId && x.Durum == input.Durum :
              x.HesapTuru == input.HesapTuru && x.SubeId == input.SubeId && x.Durum == input.Durum);
        ...
        mappedDtos.ForEach(x =>
        {
            ...
            x.Borc = x.MakbuzHareketler.Where(y => y.BelgeDurumu == BelgeDurumu.TahsilEdildi ||
              y.OdemeTuru == OdemeTuru.Pos && y.BelgeDurumu == BelgeDurumu.Portfoyde).Sum(y => y.Tutar);

            x.Alacak = x.MakbuzHareketler.Where(y => y.BelgeDurumu == BelgeDurumu.TahsilEdildi).Sum(y => y.Tutar);
