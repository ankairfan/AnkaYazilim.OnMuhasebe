...
public class OzelKodManager : DomainService
{
    ...
    public async Task CheckCreateAsync(string kod, OzelKodTuru? ozelKodTuru, KartTuru? kartTuru)
    {
        await _ozelKodRepository.KodAnyAsync(kod, x => x.Kod == kod && x.KodTuru == ozelKodTuru && x.KartTuru == kartTuru);
    }
    public async Task CheckUpdateAsync(Guid id, string kod, OzelKod entity)
    {
        await _ozelKodRepository.KodAnyAsync(kod, x => x.Id != id && x.Kod == kod && x.KodTuru == entity.KodTuru && x.KartTuru == entity.KartTuru,
            entity.Kod != kod);
    }
    public async Task CheckDeleteAsync(Guid id)
    {
        await _ozelKodRepository.RelationalEntityAnyAsync(x =>
